    ...
    internal class SceneModule : Module, ISceneModule
    {
            ...
            bool gcCollect = true, Action<float> progressCallBack = null)
        {
            ...
            if (sceneMode == LoadSceneMode.Additive)
            {
                if (_subScenes.TryGetValue(location, out SceneHandle subScene))
                {
                    Log.Warn($"Could not load subScene while already loaded. Scene: {location}");
                    return;
                }
            ...
            else
            {
                if (_currentMainScene is { IsDone: false })
                {
                    Log.Warn($"Could not load MainScene while loading. CurrentMainScene: {_currentMainSceneName}.");
                    return;
                }
        ...
        public bool ActivateScene(string location)
        {
            ...
            Log.Warn($"IsMainScene invalid location:{location}");
            return false;
        }
        ...
        public async UniTask<bool> UnloadAsync(string location, Action<float> progressCallBack = null)
        {
            ...
            if (subScene != null)
            {
                ...
                if (!_handlingScene.Add(location))
                {
                    Log.Warn($"Could not unload Scene while loading. Scene: {location}");
                    return false;
                }
            ...
            Log.Warn($"UnloadAsync invalid location:{location}");
            return false;
        }
