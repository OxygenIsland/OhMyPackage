...
namespace GameFramework
{
    ...
    public sealed class LocalizationManager : MonoBehaviour, IResourceManager_Bundles
    {
        ...
        public async UniTask LoadLanguage(string language, bool setCurrent = false, bool fromInit = false)
        {
            ...
            if (!useRuntimeModule)
            {
                Log.Warn($"禁止在此模式下 动态加载语言 {language}");
                return;
            }
#endif
            ...
            else
            {
                ...
                {
                    Log.Warn($"请使用I2Localization.asset导出CSV创建内置多语言.");
                    return;
                }
        ...
        public bool SetLanguage(string language, bool load = false)
        {
            if (!CheckLanguage(language))
            {
                ...
                Log.Warn($"当前没有这个语言无法切换到此语言 {language}");
                return false;
            }
        ...
        public bool SetLanguage(int languageId)
        {
            ...
            {
                Log.Warn($"Error languageIndex. Could not set and check {languageId}  Language.Count = {allLanguage.Count}.");
                return false;
            }
